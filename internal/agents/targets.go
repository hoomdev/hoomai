...
var roles = []Role{
	{Slug: "orquestador", Native: "hoom-orquestador", File: "00-orquestador.md",
		Desc:     "Agente principal hoomAI: rutea el trabajo, delega a los subagentes y exige hoom verify + hoom check antes de entregar. NUNCA edita codigo. Deja: nada propio; lo que cuenta lo dejan los roles que delega.",
	...
	{Slug: "arquitecto", Native: "hoom-arquitecto", File: "01-arquitecto.md",
		Desc:     "Produce el spec de una tarea en .hoom/specs (7 secciones) a partir de la vision y el pedido. Usar ANTES de implementar cualquier cambio sustancial. Escribe solo en .hoom/specs y no ejecuta comandos. Deja: .hoom/specs/<slug>.md que pasa spec_lint.",
	...
	{Slug: "designer", Native: "hoom-designer", File: "02-designer.md",
		Desc:     "Traduce el visual elegido a un UI-spec y protege el design system. Usar en tareas con interfaz de usuario. Escribe solo en .hoom/specs y no ejecuta comandos. Deja: el UI-spec en .hoom/specs/.",
		ReadOnly: false, Exec: false, Primary: false, Scope: ScopeSpecs, Isolated: false},
	{Slug: "scout", Native: "hoom-scout", File: "03-scout.md",
		Desc:     "Explora el codigo y devuelve un resumen comprimido con rutas exactas y firmas. Usar cuando entender un flujo requiere leer 4 o mas archivos. Solo lectura. Deja: nada en disco; su entrega es el resumen que devuelve.",
	...
	{Slug: "writer", Native: "hoom-writer", File: "04-writer.md",
		Desc:     "UNICO agente que edita codigo. Implementa exactamente el scope del spec y corre hoom verify al terminar. Uno solo por tarea. Deja: el codigo que pone verde hoom verify --spec (el veredicto en .hoom/verdicts/).",
	...
	{Slug: "test-writer", Native: "hoom-test-writer", File: "05-test-writer.md",
		Desc:     "Escribe tests adversariales SOLO desde el spec, sin ver jamas la implementacion. Usar tras aprobar el spec, antes o en paralelo del writer. Deja: tests que citan cada CA-n del spec (spec_trace).",
	...
	{Slug: "reviewer", Native: "hoom-reviewer", File: "06-reviewer.md",
		Desc:     "Revisa un diff con la lente asignada (readability, reliability, resilience o risk); las 4 lentes si toca seguridad, dinero o supera 400 lineas. Solo lectura de codigo; registra hallazgos con hoom finding add. Deja: hallazgos en .hoom/findings/ y el registro de review en .hoom/reviews/.",
	...
	{Slug: "characterizer", Native: "hoom-characterizer", File: "07-characterizer.md",
		Desc:     "Genera characterization tests que fijan el comportamiento ACTUAL de codigo legacy antes de refactorizar. Deja: characterization tests que fijan el comportamiento actual.",
	...
	{Slug: "analista", Native: "hoom-analista", File: "08-analista.md",
		Desc:     "Convierte los documentos del cliente en .hoom/intake en la vision (.hoom/specs/00-vision.md) y el backlog (.hoom/specs/backlog.md). Escribe solo en .hoom/specs y no ejecuta comandos. Deja: .hoom/specs/00-vision.md y .hoom/specs/backlog.md.",
		ReadOnly: false, Exec: false, Primary: false, Scope: ScopeSpecs, Isolated: false},
	{Slug: "refutador", Native: "hoom-refutador", File: "09-refutador.md",
		Desc:     "Intenta REFUTAR los hallazgos abiertos con evidencia deterministica (correr el test, citar la linea) antes de que se corrijan; maximo 2 ciclos y escala al humano. Solo lectura de codigo; cierra con hoom finding resolve. Deja: resoluciones con evidencia (.hoom/findings/<id>.res.json).",
		ReadOnly: true, Exec: true, Primary: false, Scope: ScopeEvidencia, Isolated: false},
}
